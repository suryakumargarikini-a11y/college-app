/**
 * QRCode.js v1.0.0
 * Pure JavaScript HTML5 Canvas QR Code Generator
 * Auto-calculates QR Version for any token length
 * Standard ISO/IEC 18004 Quiet Zone Margin (4 modules)
 * MIT License
 */
(function(root, factory) {
    ...
    QRPolynomial.prototype = {
        ...
        multiply: function(e) {
            ...
            for (var i = 0; i < this.getLength(); i++) num[i] = 0;
    ...
    function getRSBlocks(typeNumber, errorCorrectLevel) {
        var rsBlockRow = RS_BLOCK_TABLE[(typeNumber - 1) * 4 + errorCorrectLevel];
        if (!rsBlockRow) throw new Error("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectLevel:" + errorCorrectLevel);
        var list = [];
        for (var i = 0; i < rsBlockRow.length; i += 3) {
            var count = rsBlockRow[i];
            var totalCount = rsBlockRow[i + 1];
            var dataCount = rsBlockRow[i + 2];
    ...
    QRCodeModel.getLostPoint = function(qrModel) {
    ...
    /**
     * QRCode render entry point for HTML5 Canvas or Element
     * Standard ISO/IEC 18004 Quiet Zone Margin (4 modules)
     */
    function QRCode(element, options) {
        var opts = typeof options === 'string' ? { text: options } : (options || {});
        var text = opts.text || opts.value || '';
        var width = opts.width || opts.size || 320;
        var height = opts.height || opts.size || 320;
        var margin = opts.margin !== undefined ? opts.margin : 4; // 4 modules quiet zone
        var colorDark = opts.colorDark || opts.foreground || '#000000';
        var colorLight = opts.colorLight || opts.background || '#ffffff';
        var correctLevel = opts.correctLevel !== undefined ? opts.correctLevel : 1; // Level L=1, M=0

        var targetEl = typeof element === 'string' ? document.getElementById(element) : element;
        if (!targetEl) return;

        var canvas = targetEl.tagName && targetEl.tagName.toLowerCase() === 'canvas' ? targetEl : targetEl.querySelector('canvas');
        ...
        if (canvas && canvas.getContext) {
            var qrModel = new QRCodeModel(-1, correctLevel);
            ...
            var tileW = width / totalModules;
            var tileH = height / totalModules;
            ...
            for (var row = 0; row < moduleCount; row++) {
                for (var col = 0; col < moduleCount; col++) {
                    if (qrModel.isDark(row, col)) {
                        var x = Math.round((col + margin) * tileW);
                        var y = Math.round((row + margin) * tileH);
                        var w = Math.ceil((col + margin + 1) * tileW) - x;
                        var h = Math.ceil((row + margin + 1) * tileH) - y;
